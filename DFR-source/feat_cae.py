...
class FeatCAE(nn.Module):
    ...
    def __init__(self, in_channels=1000, latent_dim=50, is_bn=True):
        ...
        layers = []
        layers += [nn.Conv2d(in_channels, (in_channels + 2 * latent_dim) // 2,
        kernel_size=1, stride=1, padding=0)]
        if is_bn:
            layers += [nn.BatchNorm2d(num_features=(in_channels + 2 *
            latent_dim) // 2)]
        layers += [nn.ReLU()]
        layers += [nn.Conv2d((in_channels + 2 * latent_dim) // 2, 2 *
        latent_dim, kernel_size=1, stride=1, padding=0)]
        if is_bn:
            layers += [nn.BatchNorm2d(num_features=2 * latent_dim)]
        layers += [nn.ReLU()]
        layers += [nn.Conv2d(2 * latent_dim, latent_dim, kernel_size=1,
        stride=1, padding=0)]
        ...
        layers = []
        layers += [nn.Conv2d(latent_dim, 2 * latent_dim, kernel_size=1,
        stride=1, padding=0)]
        if is_bn:
            layers += [nn.BatchNorm2d(num_features=2 * latent_dim)]
        layers += [nn.ReLU()]
        layers += [nn.Conv2d(2 * latent_dim, (in_channels + 2 * latent_dim) //
        2, kernel_size=1, stride=1, padding=0)]
        if is_bn:
            layers += [nn.BatchNorm2d(num_features=(in_channels + 2 *
            latent_dim) // 2)]
        layers += [nn.ReLU()]
        layers += [nn.Conv2d((in_channels + 2 * latent_dim) // 2, in_channels,
        kernel_size=1, stride=1, padding=0)]
        # layers += [nn.ReLU()]
...
class FeatCAE3x3(nn.Module):
    ...
    def __init__(self, in_channels=1000, latent_dim=50, is_bn=True):
        ...
        # if 1x1 conv to reconstruct the rgb values, we try to learn a linear
        # combination
        # of the features for rgb
        layers = []
...
class FeatSCAE(nn.Module):
    ...
    def __init__(self, in_channels=1000, latent_dim=50):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(in_channels, in_channels2, kernel_size=1, stride=1, padding=0, groups=8),
            nn.BatchNorm2d(num_features=in_channels2),
            nn.ReLU(inplace=True))
        self.channel_shuffle1 = ChannelShuffle(groups=8)
        
        self.conv2 = nn.Sequential(nn.Conv2d(in_channels2, 2 * latent_dim, kernel_size=1, stride=1, padding=0, groups=4),
        ...
        # Decoder
        self.conv3 = nn.Sequential(nn.Conv2d(latent_dim, 2 * latent_dim, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(num_features=2 * latent_dim),
            nn.ReLU(inplace=True))
        
        # 2 * latent_dim should be a multiple of the number of groups 4, if
        # latent_dim is a multiple of 2 then it satisfies that condition
        self.conv4 = nn.Sequential(nn.Conv2d(2 * latent_dim, (in_channels + 2 * latent_dim) // 2, kernel_size=1, stride=1, padding=0, groups=4),
            nn.BatchNorm2d(num_features=(in_channels + 2 * latent_dim) // 2),
            nn.ReLU(inplace=True))
        self.channel_shuffle4 = ChannelShuffle(groups=4)
        
        # (in_channels + 2 * latent_dim) // 2 should be a multiple of the
        # number of groups 8
